# Definitional proc to organize widgets for parameters.
proc init_gui { IPINST } {
  ipgui::add_param $IPINST -name "Component_Name"
  #Adding Page
  set Page_0 [ipgui::add_page $IPINST -name "Page 0"]
  ipgui::add_param $IPINST -name "DATA_WIDTH" -parent ${Page_0}
  ipgui::add_param $IPINST -name "Lambda" -parent ${Page_0}
  ipgui::add_param $IPINST -name "Radius" -parent ${Page_0}


}

proc update_PARAM_VALUE.DATA_WIDTH { PARAM_VALUE.DATA_WIDTH } {
	# Procedure called to update DATA_WIDTH when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.DATA_WIDTH { PARAM_VALUE.DATA_WIDTH } {
	# Procedure called to validate DATA_WIDTH
	return true
}

proc update_PARAM_VALUE.Lambda { PARAM_VALUE.Lambda } {
	# Procedure called to update Lambda when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.Lambda { PARAM_VALUE.Lambda } {
	# Procedure called to validate Lambda
	return true
}

proc update_PARAM_VALUE.Radius { PARAM_VALUE.Radius } {
	# Procedure called to update Radius when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.Radius { PARAM_VALUE.Radius } {
	# Procedure called to validate Radius
	return true
}


proc update_MODELPARAM_VALUE.DATA_WIDTH { MODELPARAM_VALUE.DATA_WIDTH PARAM_VALUE.DATA_WIDTH } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.DATA_WIDTH}] ${MODELPARAM_VALUE.DATA_WIDTH}
}

proc update_MODELPARAM_VALUE.Lambda { MODELPARAM_VALUE.Lambda PARAM_VALUE.Lambda } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.Lambda}] ${MODELPARAM_VALUE.Lambda}
}

proc update_MODELPARAM_VALUE.Radius { MODELPARAM_VALUE.Radius PARAM_VALUE.Radius } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.Radius}] ${MODELPARAM_VALUE.Radius}
}

